...
def unet_train():
    ...
    sgd = tf.keras.optimizers.SGD(lr=1e-4, decay=1e-6, momentum=0, nesterov=True) #根据自己需要进行调整，我这里选择的是SGD
    ...
    print("开始训练u-net")
    model.fit(X_train, y_train, epochs=100, batch_size=12)#epochs和batch_size看个人情况调整，batch_size不要过大，否则内存容易溢出
    #我6G显存也只能设置12以下，我训练最终loss降低至350左右，acc约96.5%左右
    model.save('unet_3_new.h5')
    print('unet.h5保存成功!!!')
